...
| 経路 | 試行 | 実測 |
|---|---|---|
| `/api/feedback` 経由（＝ユーザーが体感する値） | 4回 | 22.16 / **26.13** / 20.85 / 21.94 秒 |
| SDK 直叩き（Route Handler のオーバーヘッドを除く） | 2回 | 18.60 / 19.21 秒 |

**4回中1回が `FEEDBACK_TIMEOUT_MS = 25000` を超えた。**
超えた回は AbortError となり、正常な応答が返っていたにもかかわらず pending 方式に落ちる。
pending 方式は仕様どおりの縮退なのでアプリは壊れないが、
**4回に1回カードが埋まらない**のは体感として無視できない。

この測定は `next dev`（Turbopack・開発モード）で行っており、本番ビルドや Vercel 上の値ではない。
対応方針（タイムアウト値の引き上げ／`AI_MODEL` を Haiku 4.5 に切替／現状維持）は未決定。
STEP 7 の本番確認で再測定してから判断する。

**レイテンシ対策の第一手はタイムアウト値ではなく、アプリ側計算の結果を先に出す画面設計である（2026-08-16）。**
SCORE・正解率・即答率・cause の内訳はすべてアプリ側の計算なので AI を待たずに描画できる。
AI が要るのは復習カードの文章だけなので、結果を即座に出してカードだけ後から差し込めば、
26秒でも体感は変わらない。詳細は STEP 5 で詰める。
...
- **`claude-sonnet-5` のレイテンシは 2026-08-16 に実測した（§10-11）。** `/api/feedback` 経由で 20.85〜26.13 秒（4回）。**4回中1回が 25秒のクライアント打ち切りを超えた。**
  ただしこれは `next dev`（開発モード・ローカル）の値であり、**本番ビルドや Vercel 上の値は依然として未測定**。
  STEP 7 で再測定し、必要なら Haiku 4.5 と比較して `AI_MODEL` で切り替える
