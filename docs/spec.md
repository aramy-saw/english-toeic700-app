...
- カード側の表示：「まだ説明がありません」（2026-08-18 変更）
...
- クライアント側にも `AbortController` で**45秒**のタイムアウトを置き、超えたら pending 方式に落ちる（`FEEDBACK_TIMEOUT_MS`。値の根拠は §10-12 判断2 と §12-6 f）
...
**毎回5件以上まちがえ続けると、古い `pending` はいつまでも埋まらない。**
今回の対象だけで上限5枚を使い切るため、`pending` は永久に押し出され続けうる。
「次回の呼び出しで埋まる」という保証は**なくなった**。

受け皿は3つ。いずれも仕様として許容する。

- カードの文言を「まだ説明がありません」に変えた（§10-6）。次回で埋まると予告しない
- `/review` で**手動削除**できる（§12-8）。読めない長さになったら自分で消せる
- 即答で正解すれば**卒業**してカード自体が消える（§8）。学習が進めば `pending` は自然に減る
...
#### d. カードは5枚同時に到着する（ストリーミングしない）

**「後から差し込む」の実態は「28秒後に5枚同時に出る」である。** 1枚ずつは流れてこない。

理由：ストリーミングは検証層（§10-5 V1〜V6）と相性が悪い。
**V2 の幻覚チェックは全件揃ってから行う設計**で、1枚ずつ流すと検証が後追いになり、
不正な `id` が一瞬表示される可能性がある。締切までの日数を考えてもここは複雑にしない。
...
#### f. `FEEDBACK_TIMEOUT_MS = 45000`（§10-12 判断2 の保留を解除）
...
本番実測の最大は 28.88 秒。`maxDuration = 60` なので 45 秒ならサーバー側が先に落ちることもない。
25000 のままだと5回中3回がカード未充足になるが、45000 なら実測範囲はすべて通る。
