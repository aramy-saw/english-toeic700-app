...
いまは `FEEDBACK_TIMEOUT_MS = 25000` のクライアント打ち切りが先に働くため 504 は表面化しないが、
**判断2でタイムアウト値を上げた瞬間に顔を出す構造**だった。順序として先に潰した。

**判断2：`FEEDBACK_TIMEOUT_MS` は 25000 のまま据え置く（保留）**

適切な値は画面設計に依存するため、STEP 5 の設計が固まってから決める。

| 画面設計 | 妥当なタイムアウト |
|---|---|
| 待機画面で見つめさせる | 25秒でも長い |
| 結果を先に出し、カードを後から差し込む | 45秒でも構わない |

設計より先に数値を決めると順序が逆になる。**いまは変えない。**
その結果として、現状は 4回中2回がカード未充足（pending のまま）になる。
これは仕様どおりの縮退であり、アプリは壊れない（§10-7・§11）。
