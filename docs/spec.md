...
| # | 対象 | 内容 |
|---|---|---|
| 1 | 復習カードの到着 | `opacity 0→1` ＋ `translateY 6px→0`／240ms／ease-out／**0.1秒間隔で1枚ずつ**（§12-6 e） |
| 2 | 選択肢の押下 | 背景色 120ms |

**これ以外を動かさない。** スピナー・スケルトンのシマー・パルス・数値のカウントアップは置かない（§12-6 e）。
§13「凝る場所は1箇所だけ」に対して、**1は凝る側・2は機能する最低限**という配分。

`prefers-reduced-motion: reduce` のとき：遅延と移動を無効化し、カードは即座に表示する（不透明度も遷移させない）。
